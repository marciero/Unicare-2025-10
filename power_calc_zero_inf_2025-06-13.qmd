...
- Simulations using zero inflated models as a generating process. These were validated based on analysis as summarized in `mod_summary_2025-010` and `zer0_infl_admissions_models_short_2025-10`. 
- We use parameters obtained from the zero inflated Poisson (ZIP) and zero inflated negative binomial (ZINB) models found in `mod_summary_2025-06-03`; that is, from the preliminary data of June 2025. 
- For the zero inflated Poisson model we use the model specification below. The negative binomial model is similar, with an estimated (not modelled) dispersion parameter
...
As seen in `mod_summary_2025-06-03`, and confirmed by analysis in 2025-10 documents, these seem to reasonable approximations of the generating process. We proceed as follows
...
**`mean`**: Reduction of the mean in admissions counts for `ibis`, as decimal.

**`zero prob`**: Percent increase in the probability of zero admissions for `ibis`, as a decimal. For this effect, the patient is assumed 60 years old and has both `chd` and `atrial_fibrillation`.
...
Here are the tabulated results from that last scenario
...
sim_results_pois_pois %>%
  mutate(eff_size_zero_raw = (c1 + (1 - c1) * exp(-exp(lambda_ibis + c2))) /
           (c1 + (1 - c1) * exp(-exp(c2))),
         eff_size_zero = round(eff_size_zero_raw - 1, 3)) %>% 
  select(n, eff_size, eff_size_zero) %>% 
   kbl() %>% 
