...
For example, comparing the ZIP3, and ZINB3, which have the same covariates and model specification, except with the latter having an additional dispersion parameter on the count portion of the model, the p-value on the likelihood ratio test is `r pval `, supporting the use of the negative binomial model.
